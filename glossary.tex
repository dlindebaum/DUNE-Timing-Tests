...
\newduneabbrev{amc}{AMC}{advanced mezzanine card}{Holds digitizing electronics and lives in \gls{utca} crates}

% AMC FMC carrier
\newduneabbrev{afc}{AFC}{AMC FMC Carrier}{Board slottable into a \gls{utca} crate which can mount an \gls{amc}}

% Cosmic Ray Trigger
\newduneabbrev{crt}{CRT}{Cosmic Ray Trigger}{Type of firmware for cosmic ray trigger which needs to record and timestamp received pulses}

% Dune timing system
\newduneabbrev{dts}{DTS}{DUNE Timing System}{The timing and synchronization system for DUNE. An umbrella term encompassing both the vertical drift and horizontal drift detector module timing systems}

% CCM
\newduneword{ccm}{CCM}{Control, Configuration and Management}

% CDR chip
\newduneword{cdr}{CDR}{Clock and Data Recovery}

%fib interface board
\newduneabbrev{fib}{FIB}{Fibre Interface Board}{interface to the timing endpoints}

%field programmable gate array
\newduneabbrev{fpga}{FPGA}{Field Programmable Gate Array}{An integrated circuit technology that allows the hardware to be reconfigured to execute different algorithms after its manufacture and deployment}
...
% IPMI
\newduneabbrev{ipmi}{IPMI}{Intelligent Platform Management Interface}{A set of computer interface specifications for an autonomous computer subsystem management and monitoring} 

% microTCA Interface Board
\newduneabbrev{mib}{MIB}{\gls{utca} Interface Board}{Controller of a \gls{utca} crate}

% Multiplexer
\newduneabbrev{mux}{MUX}{Multiplexer channel}{Multiplexer controlling which \gls{sfp} is active on a board with multiple \gls{sfp}s.}

% Top Detector Electronics
\newduneabbrev{tde}{TDE}{Top Detector Electronics} {Detector hardware used in the upper detector of the vertical drift module}

% Vertical Drift
\newduneabbrev{vd}{VD}{Vertical Drift} {DUNE far detector module in which the drift direction is parallel with earth gravitational field}

% White Rabbit
\newduneabbrev{wr}{WR}{White Rabbit}{An implementation of the IEEE-1588 standard. It is used to forward the clock signal and time-of-day reference data from the DUNE timing system to the \gls{vd} readout}

% HSI
\newduneabbrev{hsi}{HSI}{Hardware Signals Interface} {System in DUNE which timestamps external electrical signals}

% Small form-factor pluggable transceiver
\newduneabbrev{sfp}{SFP}{Small Form-Factor Pluggable} {A hot-pluggable transceiver for carrying Gigabit/s serial signals. The transport medium is usually an optical fibre.}

% LOS
\newduneword{los}{LOS}{Loss Of Signal}

% LOL
\newduneword{lol}{LOL}{Loss Of Lock}

% OF
\newduneword{of}{OF}{Other Frequency}

% PLL
\newduneabbrev{pll}{PLL}{Phase-Locked Loop} {A feedback circuit designed to allow one circuit board to synchronize the phase of its on board clock with an external timing signal.}

% TDC
\newduneabbrev{tdc}{TDC}{Time to Digital Converter} {A chip which assigns a timestamp to incoming pulses}

% TLU
\newduneword{tlu}{TLU}{Trigger Logic Unit}

%Micro Telecommunications Computing Architecture
\newduneabbrev{utca}{$\mu$TCA}{Micro Telecommunications Computing Architecture}{The computer architecture specification followed by the crates that house charge and light readout electronics for the \gls{dts}} %A computer architecture. Vertical card-edge connectors, Amphenol ICC }%,  \url{https://www.amphenol-icc.com/product-series/micro-tca-card-edge.html}}
...
%
%\newduneabbrev{mch}{MCH}{MicroTCA Carrier Hub}{Controller of a \gls{utca} crate}
%
