...
As part of the \dword{dts} prototyping process, a Schroff 11890-170 \dword{utca} crate, and an \dword{afc} board [2] were purchased.
The AFC is being considered to be the \dword{amc} which hosts the \dword{fib}.
...
\subsubsection{\dword{utca} crate-\dword{afc}}
...
\begin{itemize}
    ...
    \item Configuration of \dword{afc} clock routing circuity using Intelligent Platform Management Interface (IPMI) commands.
    \item Configuration of \dword{afc} on-board oscillator using IPMI commands.
\end{itemize}

\subsubsection{AFC-Timing \dword{fmc}}
...
Since this test a new protocol has been developed which does not require the \dword{cdr} chip (see section \ref{sec:new-protocol}).
...
% This overlaps with ProtoDUNE-I tests, but appears 10x better...
...
\begin{itemize}
    ...
    \item Overlord - legacy name for master: Available on \dword{fmc} and \dword{tlu}.
